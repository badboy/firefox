...
class Object(Metric):
    ...
    def __init__(self, *args, **kwargs):
        self._generate_structure = self.validate_structure(kwargs.pop("structure", None))
        super().__init__(*args, **kwargs)
    ...
    def _validate_substructure(structure):
        if not isinstance(structure, dict):
            raise ValueError(f"Nested structure needs to be an object, got: {type(structure)}")

        extra = set(structure.keys()) - Object.ALLOWED_TOPLEVEL
        if extra:
            extra = ", ".join(extra)
            allowed = ", ".join(Object.ALLOWED_TOPLEVEL)
            raise ValueError(f"Found additional fields: {extra}. Only allowed: {allowed}")

        type = structure.get("type")
        if not type:
            raise ValueError("`type` required for nested structure")

        if type not in Object.ALLOWED_TYPES:
            allowed = ", ".join(Object.ALLOWED_TYPES)
            raise ValueError(f"Invalid type: {type}. Only allowed: {allowed}")
